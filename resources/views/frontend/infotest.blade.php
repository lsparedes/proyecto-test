                        ...
                        <div class="text-center my-4">
                            <div class="header-container">
                                <div class="text-container">
                                    ...
                                    <h1 class="display-5 fw-bolder text-white mb-2">{{ $test->tipoTest->num_test }} - {{ $test->tipoTest->descripcion}}</h1>
                                    ...
                                    <p class="lead text-white-50 mb-1">
                                        ...
                                        @if (!empty($test->tipoTest->link_fuente))
                                            <ul id="fuenteDropdown" class="fuente-dropdown">
                                                <li><a href="{{ $test->tipoTest->link_fuente }}" target="_blank">Ver fuente</a></li>
                                            </ul>
                                        @endif
                                    </p>
                                </div>
                ...
                <div class="container mb-5">
                    <div class="row justify-content-center">
                        @if (!empty($test->url_test))
                            <div class="col-auto">
                                <a href="/{{ $test->url_test }}?id_participante={{ $IDparticipante }}" target="_blank" class="col">
                                    <button type="button" class="btn btn-outline-light btn-lg btn-block">
                                        <i class="fas fa-play"></i> <span>{{ $test->nombre_url }}</span>
                                    </button>
                                </a>
                            </div>
                        @endif
                        @if (!empty($test->url_adicional))
                            <div class="col-auto">
                                <a href="/{{ $test->url_adicional }}?id_participante={{ $IDparticipante }}" target="_blank" class="col">
                                    <button type="button" class="btn btn-outline-light btn-lg btn-block">
                                        <i class="fas fa-play"></i> <span>{{ $test->nombre_url_opcional }}</span>
                                    </button>
                                </a>
                            </div>
                        @endif
                        @if (!empty($test->link_millisecond))
                            <div class="col-auto">
                                <a href="/{{ $test->link_millisecond }}?id_participante={{ $IDparticipante }}" target="_blank" class="col">
                                    <button type="button" class="btn btn-outline-light btn-lg btn-block">
                                        <i class="fas fa-play"></i> <span>Reconocimiento</span>
                                    </button>
                                </a>
                            </div>
                        @endif
                        @if (!empty($test->link_millisecond2))
                            <div class="col-auto">
                                <a href="/{{ $test->link_millisecond2 }}?id_participante={{ $IDparticipante }}" target="_blank" class="col">
                                    <button type="button" class="btn btn-outline-light btn-lg btn-block">
                                        <i class="fas fa-play"></i> <span>{{ $test->nombre_url_opcional }}</span>
                                    </button>
                                </a>
...
<!-- Script para manejar el ID Participante -->
<script>
    document.addEventListener('DOMContentLoaded', function() {
