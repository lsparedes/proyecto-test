/*  jspsych-xabposition.js
 *	this plugin controls the position left right of a and b (not randomized)
 *
...
(function($) {
  jsPsych.xabpositionleft = (function() {
    ...
    plugin.create = function(params) {
      ...
      for (var i = 0; i < trials.length; i++) {
        ...
        trials[i].pos = params.pos;
        trials[i].left_key = params.left_key || 87; // defaults to 'w'
        trials[i].right_key = params.right_key || 69; // defaults to 'e'
        // timing parameters
    ...
    plugin.trial = function(display_element, trial) {
      ...
      function showSecondStimulus() {
        ...
        // NOT randomize whether the target is on the left or the right
        if (trial.pos == 1) {
        var images = [trial.a_path, trial.b_path];
        var target_left = true  ;
        }
        else if (trial.pos == 2) {
        var images = [trial.b_path, trial.a_path];
        ...
        var end_trial = function(info) {
          ...
          var trial_data = {
            "rt": info.rt,
           // "correct": info.correct, //so i shouldnt save any value for correct for xab plugins
