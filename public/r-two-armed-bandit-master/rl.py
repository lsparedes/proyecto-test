...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\44797\\Documents\\PsychoPy_Job\\Forum\\builder\\ReinforcementLearning\\rl.py',
...
green_example = visual.ImageStim(
    ...
    ori=0, pos=(-0.2, -0.25), size=(0.20, 0.25),
...
blue_example = visual.ImageStim(
    ...
    ori=0, pos=(0.2, -0.25), size=(0.2, 0.25),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
leftamount = visual.TextStim(win=win, name='leftamount',
    text='25',
    font='Arial',
    pos=(-0.2, -0.25), height=0.04, wrapWidth=None, ori=0, 
...
for thisOuterLoop in outerLoop:
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        TrialsComponents = [question_mark, blue, green, response, blue_reward, green_reward, purple_marker, gold_marker, points_marker]
        ...
        feedback_text.setHeight(cash_size)
        fb_blue.setPos((pb, 0))
        ...
        # keep track of which components have finished
        FeedbackComponents = [fb_sound, fb_correct, feedback_text, red_frame, fb_blue, fb_green, fb_blue_reward, fb_green_reward, purple, gold, points]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if feedback_text.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
                # keep track of start time/frame for later
            ...
            if feedback_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > feedback_text.tStartRefresh + 4-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if red_frame.status == STARTED:  # only update if drawing
                red_frame.setPos((px, 0), log=False)
            ...
            if fb_green.status == STARTED:  # only update if drawing
                fb_green.setPos((pg, 0), log=False)
